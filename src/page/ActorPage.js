...
const ActorPage = () => {
  ...
  const [url, setUrl] = useState(tmdbAPI.getActorList("popular", nextPage));
  ...
  useEffect(() => {
    if (filterDebounce) {
      setUrl(tmdbAPI.getActorSearch(filterDebounce, nextPage));
    } else {
      setUrl(tmdbAPI.getActorList("popular", nextPage));
    }
      ...
      <div className="select-none py-28 page-container wrapp-list--page">
        ...
        <div className="grid grid-cols-4 gap-10 wrapp-list--item">
          {!loading &&
            movies.length > 0 &&
            movies.map((item) => (
              <ActorCardList key={item.id} item={item}></ActorCardList>
            ))}
        </div>
